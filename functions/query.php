...
function querySelect_buscaApontamento($idUsuario, $data, $srvCampo = 'N') {
    ...
    $query = "SELECT LOG_APONTAMENTO.*, TB_LOGIN.NOME,LOG_APONTAMENTO.SECAO_APONT, TB_LOGIN.PSECAO_DESCRICAO, PARTE.DESCRICAO 'PARTE_DESC', ATIVIDADE.DESCRICAO 'ATIV_DESC' FROM LOG_APONTAMENTO (NOLOCK) INNER JOIN TB_LOGIN ON TB_LOGIN.LOGIN = LOG_APONTAMENTO.ID_USUARIO LEFT JOIN $base->nomeBaseRM.DBO.GCONSIST PARTE ON PARTE.CODTABELA = 'PARTE' AND PARTE.CODCLIENTE = LOG_APONTAMENTO.PARTE COLLATE SQL_Latin1_General_CP1_CI_AS LEFT JOIN $base->nomeBaseRM.DBO.GCONSIST ATIVIDADE ON ATIVIDADE.CODTABELA = 'ATIVIDADE' AND ATIVIDADE.CODCLIENTE = LOG_APONTAMENTO.ATIVIDADE COLLATE SQL_Latin1_General_CP1_CI_AS WHERE (ID_USUARIO IN (".$idUsuario.")".$campo.") AND CONVERT(date,H_INICIO,103)='".$data."'";
