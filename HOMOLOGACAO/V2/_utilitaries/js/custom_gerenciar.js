...
function interna_editApont(dadosApont, sup) {
	...
	camposEdit.forEach(ele => {
		if (ele.type == "select-one" || ele.type == 'checkbox') {
...
function js_enviaEditApont(botao, idApont, login, sup){
	...
	return $.ajax({
		...
		complete: function(data) {
			...
			js_pesquisaGerenciar(document.getElementById('inpDataFiltro').value.split('/').reverse().join('/'), login,1, sup);						
		}
	});	
}