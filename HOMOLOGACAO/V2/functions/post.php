...
function post_getApontar($idApont, $valida, $valoresEdit = "") {
    try {
        ...
        if ($idApont == "NULL") {//apontar novo
            ...
            $inpServCampo = (interna_verificaCampoNull($valoresEdit["inpServCampo"]) == "NULL")?"N":"S";
            $selCausaRetrabalho = (interna_verificaCampoNull($valoresEdit["selCausaRetrabalho"]) == "NULL")?"NA":($valoresEdit["selCausaRetrabalho"]);
            $dataFormatInicio = date_format(date_create_from_format('d/m/Y H:i', $valoresEdit["inpDataInicio"]. " ".$valoresEdit["inpHoraInicio"]), 'Y-m-d H:i:s');
            $dataFormatFim = date_format(date_create_from_format('d/m/Y H:i', $valoresEdit["inpDataInicio"]. " ".$valoresEdit["inpHoraFim"]), 'Y-m-d H:i:s');
        ...
        //recupera coligada e filial da OS
        $origemOS = querySelect_dadoOS($inpNumOs);
        ...
        if (isset($codRequisitante)) {//monta array com os valores
            array_push($insertBanco,array(
                ...
                "CODFILIAL" => $origemOS[0]["CODFILIAL"],
                "CODCOLIGADA" => $origemOS[0]["CODCOLIGADA"]
            ));
            ...
            if ($conn) {
                if (isset($apont)) {//edita/aprova apontamento              
                    ...
                    }else if ($valida == "A"){//só envia para o RM apontamento Aprovado
...
function interna_montaXML($campos) {
    ...
    $xml->TITMMOVCOMPL->NOS = substr($campos["N_OS"], -6); $campos["N_OS"];
