...
function querySelect_lista($codSecao) {
    $corporeRM = new Config();
    $query = "SELECT SUBSTRING(SECAOPARTE.CODINTERNO,5,5) 'ITEM', SUBSTRING(PARTEATIV.CODINTERNO,0,5) 'PARTE', PARTE.DESCRICAO 'DESCRICAO_PARTE', SUBSTRING(PARTEATIV.CODINTERNO,5,5) 'ATIV', ATIVIDADE.DESCRICAO 'DESCRICAO_ATIV' FROM $corporeRM->nomeBaseRM.DBO.GCONSIST SECAOPARTE (NOLOCK)  INNER JOIN $corporeRM->nomeBaseRM.DBO.GCONSIST PARTEATIV (NOLOCK) ON PARTEATIV.CODTABELA = 'PARTEATIV' AND PARTEATIV.DESCRICAO = SUBSTRING(SECAOPARTE.CODINTERNO,5,5) LEFT JOIN $corporeRM->nomeBaseRM.DBO.GCONSIST PARTE (NOLOCK) ON PARTE.CODTABELA = 'PARTE' AND PARTE.CODCLIENTE = SUBSTRING(PARTEATIV.CODINTERNO,0,5) LEFT JOIN $corporeRM->nomeBaseRM.DBO.GCONSIST ATIVIDADE (NOLOCK) ON ATIVIDADE.CODTABELA = 'ATIVIDADE' AND ATIVIDADE.CODCLIENTE = SUBSTRING(PARTEATIV.CODINTERNO,5,5)  WHERE SECAOPARTE.CODTABELA = 'SECAOPARTE' AND SUBSTRING(SECAOPARTE.CODCLIENTE,0,5) = ".$codSecao;
