			...
			if (jsonSecaoNomeSup.length > 0) {
				for (var i = 0; i < jsonSecaoNomeSup.length; i++) { //alimenta os campos do select de seção na tela
					if (campoID.querySelectorAll("[value='" + jsonSecaoNomeSup[i]['SECAO_DESCRICAO'] + "']").length == 0) { //verifica se já existe a opção no select
						if (secaoDescricao.filter(secaoDescricao => secaoDescricao.length )) {
						//if (secaoDescricao.filter((secaoDescricao, i) => secaoDescricao.indexOf(secaoDescricao)===i)) {
						//$("#" + campoID.id).append("<option value='" + jsonSecaoNomeSup[i]['SECAO_DESCRICAO'] + "'>" + jsonSecaoNomeSup[i]['SECAO_DESCRICAO'] + "</option>");
						//secaoDescricao.filter(function(elem, pos, self) { return self.indexOf(elem) == pos; })
						secaoDescricao.push("<option value='" + jsonSecaoNomeSup[i]['SECAO_DESCRICAO'] + "'>" + jsonSecaoNomeSup[i]['SECAO_DESCRICAO'] + "</option>")
						//secaoDescricao.filter(function(elem, pos, self) { return self.indexOf(elem) == pos; })									
					}  
				}	secaoDescricao.sort();	
				//$("#" + campoID.id).append(secaoDescricao);		
