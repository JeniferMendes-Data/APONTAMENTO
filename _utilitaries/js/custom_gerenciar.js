...
function interna_atualizaContagem(json, data, sup) {
	...
	json.forEach(element => { //alimenta um objeto com a seção e cada colaborador que realizou apontamento para a data filtrada
		var codSecao = element["SECAO_APONT"];
...
function interna_editApont(dadosApont, sup) {
	...
	// var dia = new Date()
	switch(hoje.toLocaleDateString('en-GB')){
		case (hoje.toLocaleDateString('en-GB')):
			if (sup == 1) {
				readOnly = true;
				required = false;
			...
			break;
		case (ontem.toLocaleDateString('en-GB')):	
			if (sup == 1) {
				readOnly = true;
				required = false;
				aprovarDisplay = 'block';
			...
			break;
		case (hoje.toLocaleDateString('en-GB') <=5):
			if (sup == 1) {
				readOnly = true;
				required = false;
				aprovarDisplay = 'block';
